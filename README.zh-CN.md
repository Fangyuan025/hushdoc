...
├── setup.bat        一次性安装器（依赖 + runtime + 模型）
├── hushdoc.bat      一键启动器（带退出清理询问）
...
- **Linux / macOS 用户：** 把 `hushdoc.bat` 换成 `./dev.sh`。自动清理
  目前只在 `.bat` / `.ps1` 流程里——`dev.sh` 能起服务但退出时不会
  弹清理询问。
